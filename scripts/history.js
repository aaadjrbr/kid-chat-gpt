...
// Fetch chat sessions for a specific day
export async function displayChatHistoryByDay(parentId, kidId, year, month, day) {
    ...
    // Query the chat sessions ordered by dateStarted in descending order
    const q = query(
        chatSessionsRef,
        where("dateStarted", ">=", startOfDay),
        where("dateStarted", "<=", endOfDay),
        orderBy("dateStarted", "desc") // Order by dateStarted in descending order
    );
    ...
    chatSnapshot.forEach(doc => {
        ...
        sessionItem.textContent = `New Chat - Started at ${new Date(session.dateStarted.seconds * 1000).toLocaleTimeString()}`;
